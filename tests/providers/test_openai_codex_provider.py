...
def _mock_codex_token(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "nanobot.providers.openai_codex_provider.get_codex_oauth_token_async",
        fake_token,
    )
...
async def test_codex_provider_passes_proxy_to_oauth_and_response_request(monkeypatch) -> None:
    ...
    async def fake_token(*, proxy=None):
    ...
    monkeypatch.setattr("nanobot.providers.openai_codex_provider.get_codex_oauth_token_async", fake_token)
    monkeypatch.setattr("nanobot.providers.openai_codex_provider._request_codex", fake_request)
